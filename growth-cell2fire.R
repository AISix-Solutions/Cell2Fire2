...
runCell2Fire <- function(numIgnitions) {
  ...
  system2(cell2fireExecutable,
          c("--input-instance-folder", inputInstanceFolder,
            ...
            "--ignitions --final-grid --Fire-Period-Length 1 --weather random"))
}
...
# GC: For some ridiculous reason the length of this file sets the max number
# of burn hours so I just set it to 100 here. 
...
generateWeatherFile <- function(weatherData, uniqueFireIndex) {
  weatherData %>%
    ...
    #fwrite(file.path(tempDir, "Weather.csv"))
...
write.table(as.matrix(fuelsRaster, wide=TRUE), file=mapMetadataFile, row.names=FALSE, col.names=FALSE,append=TRUE)
# Spatial data
